...
#A test function for the changing from psi to kpa was added to test_water_flow.py

from water_flow import water_column_height, pressure_gain_from_water_height, pressure_loss_from_pipe, pressure_loss_from_fittings, reynolds_number, pressure_loss_from_pipe_reduction, pressure_kpa_to_psi
...
def test_pressure_loss_from_pipe_reduction():
    ...
    assert pressure_loss_from_pipe_reduction(0.286870, 1.65, 471729,0.048692) == approx(-164.015, abs=0.001)
    assert pressure_loss_from_pipe_reduction(0.286870, 1.75, 500318, 0.048692) == approx(-184.487, abs=0.001)

def test_pressure_kpa_to_psi():
    assert pressure_kpa_to_psi(0.0) == approx(0.0, abs=0.1)
    assert pressure_kpa_to_psi(68.9476) == approx(10.0, abs=0.1)
    assert pressure_kpa_to_psi(175.816) == approx(25.5, abs=0.1)
