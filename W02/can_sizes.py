...
def main():
    ...
    best_efficiency = {'name': '', 'efficiency': 0}
    best_cost = {'name': '', 'efficiency': 0}
    
    for can in can_sizes:
        #initialize variables
        radius = float(can["radius"])
        height = float(can["height"])
        cost = float(can["cost"].strip('$'))
        name = can["name"]
        ...
        #output
        print(f"{name:<15}{storage_efficiency:>15.4f}{cost_efficiency:>15.4f}")
        
        #Determine best efficiencies
        if storage_efficiency > best_efficiency['efficiency']:
            best_efficiency = {'name': name, 'efficiency': storage_efficiency}
        if cost_efficiency > best_cost['efficiency']:
            best_cost = {'name': name, 'efficiency': cost_efficiency}
        
    #Best efficiencies output
    print(f"\nBest storage efficiency: {best_efficiency['name']} ({best_efficiency['efficiency']:.4f})")
